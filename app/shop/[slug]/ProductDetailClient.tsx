...
import { productFaqs } from '@/lib/schema'
...
/* ── electrical ──────────────────────────────────────────── */
function Electrical({ p, contact }: { p: Product; contact?: SiteContact | null }) {
  const salesPhone = contact?.salesPhone || DEFAULT_CONTACT.salesPhone
  return (
    <div>
      ...
      <div className="reveal" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 28 }}>
          ...
          <div>
            ...
            <div className="mono" style={{ color: 'var(--navy-500)', marginTop: 2 }}>Free with every contract · book at {salesPhone}</div>
          </div>
        </div>
...
function FAQ({ p }: { p: Product }) {
  const [open, setOpen] = useState(0)
  // Product-specific Q&As first — these mirror the FAQPage schema emitted in
  // layout.tsx (productFaqs), so visible content matches structured data per
  // Google's FAQ guidelines. The generic ones below are bonus visible content.
  const faqs = [...productFaqs(p), ...FAQS]
  return (
    <div>
...
/* ── buy box (sticky) ────────────────────────────────────── */
function BuyBox({ p, contact }: { p: Product; contact?: SiteContact | null }) {
  const [duration, setDuration] = useState(12)
  const [silencer, setSilencer] = useState(false)
  const total = p.price + (silencer && p.silencerPrice > 0 ? p.silencerPrice : 0)

  // window.location is only available client-side; populate after mount to avoid hydration mismatch.
  const [pageUrl, setPageUrl] = useState('')
  useEffect(() => {
    if (typeof window !== 'undefined') setPageUrl(window.location.href)
  }, [])

  const waNumber = digitsOnly(contact?.whatsappNumber || DEFAULT_CONTACT.whatsappNumber)
  const salesPhone = contact?.salesPhone || DEFAULT_CONTACT.salesPhone
  ...
  const waMessageLines = [
    ...
    silencer && p.silencerPrice > 0 ? `• Silencer add-on: yes (+${fmtINR(p.silencerPrice)})` : '',
    `• Total: ${fmtINR(total)}`,
    p.sku ? `• SKU: ${p.sku}` : '',
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          ...
          {(() => {
            ...
            const _ctaLabel = _s === 'In Stock' ? 'Buy Miner →' : _s === 'Sold Out' ? 'Sold Out' : 'Notify Me →'
            ...
            const dot = <span style={{ width: 8, height: 8, borderRadius: '50%', background: _soldOut ? 'rgba(251,251,243,0.3)' : 'var(--navy-900)' }} />
            if (_soldOut || !waHref) {
              return (
                <button disabled style={ctaStyle}>
                  {dot}{_ctaLabel}
                </button>
              )
            }
            return (
              <a
                ...
                aria-label={`${_ctaLabel.replace(' →', '')} — chat on WhatsApp`}
                style={ctaStyle}
          ...
          <div style={{ marginTop: 18, fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '0.1em', color: 'rgba(251,251,243,0.45)', textAlign: 'center' }}>
            Questions? Call <a href={`tel:${digitsOnly(salesPhone)}`} style={{ color: 'var(--mint-300)', textDecoration: 'none' }}>{salesPhone}</a>
          </div>
        </div>
      </div>
...
/* ── page (client island) ─────────────────────────────────
   Product data is fetched server-side in page.tsx and passed in as a prop,
   so the H1, specs and FAQ render into the initial SSR HTML (good for SEO /
   non-JS crawlers). Only client-only concerns (reveal animations, contact
   numbers for the WhatsApp deep-links) stay in useEffect here. */
